...
// Load New York data and update chart
d3.csv("./dataset/new_york_1978-2023_min.csv").then(minData => {
    d3.csv("./dataset/new_york_1978-2023_max.csv").then(maxData => {
        ...
        // Reinitialize the y2 domain with all available years
        y2.domain(years);

        // Calculate the x2 domain dynamically based on the data for New York
        const minValue = d3.min(data, d => d.Value);
        const maxValue = d3.max(data, d => d.Value);
        x2.domain([Math.min(minValue, -20), Math.max(maxValue, 50)]); // Limit the x-axis to a more realistic range
