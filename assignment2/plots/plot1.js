...
document.addEventListener("DOMContentLoaded", () => {
  ...
  d3.csv("./dataset/continent_emissions_top5_2022.csv").then(data => {
    ...
    nodes.on("mouseover", (event, d) => {
      ...
      // Debug: Log node name to check which node is hovered over
      console.log("Hovered over node:", d.name);
      ...
      if (d.name.includes("Total - Fossil") || d.name.includes("Total - Land")) {
        ...
        console.log("Aggregating for continent:", continent);

        // Sum the emissions for that continent (make sure the continent name matches correctly)
        ...
        // Debug: Log the aggregated values for continents
        console.log(`Fossil Value: ${fossilValue}, Land Value: ${landValue}, Total: ${totalValue}`);
      } else {
        ...
        // Debug: Log the values for countries
        if (countryData) {
          console.log(`Country: ${d.name}, Fossil Emissions: ${fossilValue}, Land Emissions: ${landValue}`);
        } else {
          console.log(`No data found for country: ${d.name}`);
        }
      }

      // Display tooltip with correct values
